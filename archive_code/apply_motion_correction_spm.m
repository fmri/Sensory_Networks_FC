%%% The purpose of this script is to apply spm motion correction
%%% to functional files
...
parfor ss = 1:length(subjCodes)
    ...
    for rr = 1:length(runs)
        if ~isfile([subjectsDir subjCode '/bold/00' num2str(rr) '/rf.nii'])
            rundir = [subjectsDir subjCode '/bold/00' num2str(rr) '/'];
            [files,~] = spm_select('ExtList',rundir, '^f.nii', Inf);
            cd(rundir)
            spm_realign(files);
            spm_reslice(files);
        else
    ...
    if restingstate
        ...
        for rr = 1:length(runs)
            if ~isfile([subjectsDir subjCode '/rest/00' num2str(rr) '/rf.nii'])
                rundir = [subjectsDir subjCode '/rest/00' num2str(rr) '/'];
                [files,~] = spm_select('ExtList',rundir, '^f.nii', Inf);
                cd(rundir)
                spm_realign(files);
                spm_reslice(files);
