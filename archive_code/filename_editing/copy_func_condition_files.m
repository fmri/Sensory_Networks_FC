...
for ss = 1:length(subjCodes)
    ...
    for rr = 1:length(runs)
        ...
        file_mask = contains(files,'events.tsv');
        selected_filepath = files(file_mask);

        for ii = 1:length(selected_filepath)
            new_filename = replace(selected_filepath{ii}, '_events', '_topupApplied_events');
            unix(['cp ' func_path selected_filepath{ii} ' ' func_path new_filename])
        end
    end
end