...
% Load gppi betas 
load('/projectnb/somerslab/tom/projects/sensory_networks_FC/data/gPPI_va-aa_fulltable_5networks.mat', 'data_table', 'subjCodes')
assert(all(strcmp(diff_subjCodes, subjCodes)))
N = length(subjCodes);

connections = {'abias<->pAud', 'abias<->abias', 'abias<->vbias', 'pVis<->vbias', 'pVis<->supramodal'};
N_conns = length(connections);

betas = nan(N,N_conns,2);

%% Group into aud and vis hypotheses
connection_types = {{'abias<->pAud', 'abias<->abias', 'abias<->vbias'}, {'pVis<->vbias', 'pVis<->supramodal'}};
