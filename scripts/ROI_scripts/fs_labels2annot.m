...
subjCodes = subjCodes(~ismember(subjCodes, {'AH', 'RR'})); % rejected subjs
N = length(subjCodes);
...
for ss = 1:N
    ...
    for hh = 1:length(hemis)
        ...
        % Check for replacement ROIs and make sure they are used (or not used, depending on use_replacement variable)
        ...
        if any(replacement_ROIs)
            ...
            for ii = 1:length(rep_ROI_inds)
                ...
                if use_replacements
                    ...
                    bad_nonrepl(contains(subj_labels, repl_ROI_name)) = true;
                end
            end
