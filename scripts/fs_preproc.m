...
parfor ss = 1:N
    ...
    unix(['mktemplate-sess -s ' subjCode ' -d ' data_dir ' -fsd localizer -funcstem f -update']);
    unix(['register-sess -s ' subjCode ' -d ' data_dir ' -fsd localizer -delete-dat -dof 6 -bold -per-run -update -init-coreg']);
    unix(['mkbrainmask-sess -maskstem brain -fsd localizer -s ' subjCode ' -d ' data_dir ' -per-run -update']);
