...
for ss = 1:length(subjCodes)
    ...
    if contains(FMruns, '/') % different spacetime runs may have different fieldmaps
        FMruns = str2num(replace(FMruns, '/', ',')); % still take all fieldmaps
    else
    ...
    for ff = 1:length(func_runs)
        if strcmp(subjCode, 'NS') && strcmp(experiment_name, 'spacetime') % This subj has 1st run with 1st pair of FMs then next 3 runs with 2nd pair of FMs
            ...
            if ff==1
                topup_path = [dirTarget 'bold/sub-' subjCode 'runs' num2str(FMruns(1)) num2str(FMruns(2)) fmap_prefix 'fmapTopupOut'];
            else
                topup_path = [dirTarget 'bold/sub-' subjCode 'runs' num2str(FMruns(3)) num2str(FMruns(4)) fmap_prefix 'fmapTopupOut'];
            end
        elseif ismember(subjCode, {'AG', 'AI'}) && strcmp(experiment_name, 'rest') % This subj has rs runs 1 and 2 with 1st pair of FMs then 3rd rs run with 2nd pair of FMs
            assert(num_pairs == 2, ['unexpected number of fieldmaps for subj ' subjCode ' (expected 2, got ' num2str(num_pairs)])
            if ff==3
                topup_path = [dirTarget 'bold/sub-' subjCode 'runs' num2str(FMruns(3)) num2str(FMruns(4)) fmap_prefix 'fmapTopupOut'];
            else
